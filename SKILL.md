...
Use this skill to generate compliance legal documents for a SaaS product or mobile app and export them as deliverable PDF files.

---

## Step 1 — Collect information from the user

Before writing anything, ask the user to confirm the following. **Do not infer or pull values from code — all information must be explicitly provided by the user.**
...
5. **Data storage provider and region** (e.g. Tencent Cloud Shanghai, AWS us-east-1)

**Optional (improves accuracy):**
- Any cross-border data transfers?
- Any third-party analytics or advertising SDKs? (e.g. Firebase, Google Analytics)
- Company registered address (for terms of service signature block)
...
### Document 1: Privacy Policy (`privacy-policy.md`)

Section structure (in order):
1. Introduction — product name, supported platforms (current: H5 web; note if native App is planned)
2. Information we collect — categorized list (identity, credentials, health/sensitive data, device logs)
3. How we use information — table mapping purpose to data type
4. Sensitive personal information — masking rules, extra protection measures (e.g. ID number: show first 6 + last 4 digits only)
5. Data storage — storage region, retention period table per data type
6. Data sharing — lawful sharing scenarios (service providers, legal requirements, emergencies)
...
9. Minors — state the system is intended for adult staff only
...
- Adjust applicable law section to match the user's jurisdiction (PIPL for China, GDPR for EU, CCPA for California, etc.)

### Document 2: Terms of Service (`terms-of-service.md`)
...
7. Service provider obligations — SLA (e.g. 99% uptime), data security, support, daily backups
...
**Writing notes:**
- Pricing table must include monthly / annual / multi-year columns — use only the prices confirmed by the user, never pull from code
...
### Document 3: Cookie Policy (`cookie-policy.md`)
...
- Note that cookie policy applies to the web version only; native App will be addressed separately when released

### Document 4: Data Processing Agreement (`data-processing-agreement.md`)

When to use: for B2B products where you process personal data on behalf of customers (required under GDPR Article 28 / PIPL Article 21).

Section structure:
1. Introduction — applicable law, define data controller and data processor
...
7. Data retention and deletion — after service termination: export window (30 days) + permanent deletion (within 60 days) + written deletion certificate
8. Audit rights
9. Liability allocation
10. Agreement term and dispute resolution

**Writing notes:**
- Sub-processors (e.g. Tencent Cloud, AWS, Google Cloud) must be disclosed by name in the agreement
...
### Document 5: Disclaimer (`disclaimer.md`)
...
6. Regulatory compliance — each customer is responsible for their own local compliance
7. Intellectual property notice
8. Contact information

**Writing notes:**
- Health-related products must clearly state "does not constitute medical diagnosis or medical advice"
- Include emergency handling guidance: contact emergency services first, then log the incident in the system
...
| Platform change (App released) | `privacy-policy.md` (platform section) + `cookie-policy.md` (add App section) |
...
- Chrome `--headless=new` on macOS prints `task_policy_set` warnings to stderr — these are harmless. Suppressed with `2>/dev/null`. The PDF is generated correctly.
...
- **Never hard-code pricing.** Always confirm current prices with the user before writing them into any document.
- Data deletion timeline must be consistent across all documents: **30-day export window + permanent deletion within 60 days.**
